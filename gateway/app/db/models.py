...
class UserAccountDB(Base):
    ...
    voice_profiles: Mapped[list["VoiceProfileDB"]] = relationship(back_populates="user", cascade="all, delete-orphan")
    devices: Mapped[list["DeviceRegistrationDB"]] = relationship(back_populates="user", cascade="all, delete-orphan")
    tasks: Mapped[list["TaskDefinitionDB"]] = relationship(back_populates="user", cascade="all, delete-orphan")
...
class VoiceProfileDB(Base):
    ...
    id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    user_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("user_accounts.id", ondelete="CASCADE"), nullable=False)
...
class VirtualNumberDB(Base):
    ...
    number_type: Mapped[Literal["permanent", "pooled"]] = mapped_column(String(20), default="permanent")
    assigned_user_id: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey("user_accounts.id", ondelete="SET NULL"))
    provider: Mapped[Literal["plivo", "chime"]] = mapped_column(String(20), default="plivo")
...
class SipCredentialDB(Base):
    ...
    domain: Mapped[str] = mapped_column(String(255), default="phone.plivo.com")
...
class TaskDefinitionDB(Base):
    ...
    status: Mapped[Literal["pending", "active", "completed", "failed", "stopped"]] = mapped_column(String(20), default="pending", index=True)
...
class AuditLogDB(Base):
    ...
    id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    user_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("user_accounts.id", ondelete="SET NULL"), nullable=True)
