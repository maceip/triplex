...
class VoicePromptProgressTest {
    ...
    @Test
    fun `live microphone energy visibly increases sphere alignment`() {
        val progress = voicePromptProgress(4_000)

        assertTrue(
            voicePromptAlignment(progress, amplitude = 0.8f) >
                voicePromptAlignment(progress, amplitude = 0.05f)
        )
        assertTrue(voicePromptAlignment(progress, amplitude = 10f) <= 1f)
    }
}
