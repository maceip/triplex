...
class VoiceCloneViewModel @Inject constructor(
    ...
    private suspend fun prepare(seconds: Float, preparedReference: File) {
        ...
        val result = repository.prepareFromPhrases(
            phraseWavs = listOf(preparedReference),
        ...
        _state.value = when (result) {
            is Result.Success -> {
                ...
                _state.value.copy(
                    ...
                    placement = result.data.placement.ifBlank { "LOCAL" },
                    placementReason = result.data.placement_reason,
                    referenceSeconds = seconds.toInt(),
                    message = "Voice ready on device"
                )
            }
            is Result.Error -> {
                preparedReference.delete()
