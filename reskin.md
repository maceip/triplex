...
Status: proposed, awaiting approval Scope: apps/android UI + presentation-layer wiring only. The Rust agent core, PJSIP stack, SODA ASR, Inflect TTS, and gateway contracts do not change except at the narrow seams named below.

Ground rules for this effort:

No Compose is written until this plan and the custom design system are approved. This document defines where the design system plugs in, not what it looks like.
Every screen consumes state through a ViewModel and dispatches sealed intents. No screen talks to a service companion object, a global singleton object, or a ContentProvider directly.
The two call stacks (Telecom/SIM and Plivo/SIP) stay separate below the domain layer; the UI sees exactly one call model.
...
├─ Tab: keypad (start)          — dialer home: search + history + dialpad (§3.1); never gated
├─ Tab: agent                   — agent home: previous runs + status (§3.2)
│   └─ [gate] enrollment        — replaces the agent tab root until a device token exists
│                                 (existing EnrollmentScreen); the keypad stays reachable
...
Three rules that keep telephony sane:

Enrollment gates the agent tab, not the shell. Enrollment is a gateway concern — it obtains the device token the agent needs — while this activity holds the platform dialer role. An app that owns that role and then refuses to dial is broken in the way that matters most, so a missing device token blanks the agent tab root and nothing else; keypad, recents, contacts and emergency dialling stay reachable at all times. (Earlier revisions of this section drew the gate in front of the whole shell; that predates the shell also being the dialer.) The two deeper agent routes need no guard of their own — they are only navigated to from the tab root, so the gate covers them by construction. That changes the day they gain deep links.
...
DialerCallNotificationManager + ScreeningCoordinator.showScreeningNotification (two parallel pipelines)	One CallNotificationController observing the repository: CallStyle incoming/ongoing/missed for both stacks; actions post CallIntents through a single receiver; full-screen intent targets the shell
...
speakUserReply(text: String) — public wrapper over the existing private speakAgent, used by the sheet's reply chips. Appends a UserReply turn instead of a TRIPLEX turn.
ConversationSpeaker gains USER (or the mapping happens at the source layer — decide in Phase 1 review).
Auto-answer (onSipEvent → INCOMING_CALL → answer()) is gated on AgentConfigRepository.inbound.autoAnswerAll instead of being unconditional. Default stays on ("agent answers all calls").
Greeting text and automation openings read from config instead of the hard-coded SCREENING_GREETING / when (automationId) strings.
2.4 New data layer
...
Registers ContentObservers on CallLog.Calls.CONTENT_URI and ContactsContract so recents/contacts are live flows (call log updates the moment a call ends).
...
EnrollmentScreen/EnrollmentViewModel stay the onboarding gate, unchanged, but scoped to the agent tab root rather than to the whole shell (§2.2).
...
└──────────────────────────────┘
State: IncomingCallViewModel over CallSessionRepository.session + AgentConfigRepository. All actions are CallIntents.
...
Enforcement. DesignSystemGuardTest currently scans only ui/screens. After the re-org it scans all of ui/ except ui/components and ui/theme, so the dialer and call surfaces are held to the same boundary. The forbidden-snippet list gains the raw-Material imports the dialer uses today.
...
5 — In-call + agent panel	In-call screen per §3.4 on CallIntents; agent panel with per-stack hand-off (SIP resume, deflect, conference-merge); delete InCallContent	All existing call controls work through the new surface on a real device; hand-off paths demonstrated per stack (or capability-gated off with visible reason)
...
Shell layout: two tabs (Keypad | Agent) with recents/contacts folded into keypad search — confirm, or keep a separate Recents tab for one-thumb reach.
Retire MainActivity and AgentScreeningActivity into the single shell (recommended) — confirm.
Mid-call hand-off on carrier calls: conference-merge as the wired mechanism (recommended); acoustic speaker+mic takeover stays out of scope as a spike.
Run history: local-only Room (recommended) vs. synced to the gateway.
T9 smart dial: in Phase 2, or number-prefix matching only for v1.
Room + activity-alias additions are the only manifest/dependency changes; flag anything else you want frozen.
